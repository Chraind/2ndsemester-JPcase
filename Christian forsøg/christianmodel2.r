...
# ── Optimal threshold ──────────────────────────────────────────────────────────
cat("\n--- Model 1: Optimal threshold ---\n")
roc_data <- collect_predictions(final_fit) %>%
  roc_curve(truth = churn, .pred_1)

optimal_threshold <- roc_data %>%
  mutate(dist = sqrt((1 - sensitivity)^2 + (1 - specificity)^2)) %>%
  slice_min(dist, n = 1) %>%
  pull(.threshold)

cat("Optimal threshold:", round(optimal_threshold, 3), "\n")

test_preds_optimal <- collect_predictions(final_fit) %>%
  mutate(
    .pred_class_adj = factor(
      if_else(.pred_1 >= optimal_threshold, "1", "0"),
      levels = c("1", "0")
    )
  )

test_preds_optimal %>%
  conf_mat(truth = churn, estimate = .pred_class_adj)

test_preds_optimal %>%
  metric_set(accuracy, sens, spec, f_meas)(
    truth    = churn,
    estimate = .pred_class_adj
  )
# ──────────────────────────────────────────────────────────────────────────────
...
scenarier <- tibble(
  ...
  # Numeriske - NYE behavior features
  laesedage_spredning      = c(55, 30, 0),
  andel_weekend            = c(0.25, 0.20, 0.0),
  andel_morgen             = c(0.40, 0.25, 0.0),
  andel_aften              = c(0.20, 0.15, 0.0),
  andel_ios                = c(0.30, 0.40, 0.0),
  andel_android            = c(0.20, 0.30, 0.0),
  andel_windows            = c(0.30, 0.20, 0.0),
  andel_sport              = c(0.15, 0.10, 0.0),
  andel_politik            = c(0.20, 0.15, 0.0),
  andel_oekonomi           = c(0.10, 0.05, 0.0),
  andel_kultur             = c(0.10, 0.05, 0.0),
