...
# account_active_days <- Total aktive dage - (dato_nu - sidste kampagne dag)
...
# Behold seneste observation pr. bruger
...
model_data <- merged_data %>%
  ...
  mutate(
    ...
    permission_given_order =
      factor(if_else(permission_given_order == "true", 1, 0)),
    
    permission_given_today =
      factor(if_else(permission_given_today == "true", 1, 0)),
